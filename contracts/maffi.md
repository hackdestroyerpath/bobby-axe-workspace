...
- `status` входных записей должен быть `ready`
...
- `ANALYSIS_NOT_FOUND` — нет аналитики по тикеру
- `INCONSISTENT_SIGNALS` — сигналы конфликтуют и не дают построить сетку
...
```json
{
  ...
  "sl": 63940.0,
  "tp": 64780.0,
  "rationale": "Преобладают bullish-сигналы на 5m и 60m, ожидается восстановление от зоны спроса.",
  "based_on": {
    "analysis_ids": [
