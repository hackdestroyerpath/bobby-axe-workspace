...
Эти артефакты публикуются как единый пакет на один символ/секундный cycle или как три согласованных сообщения с общим `decision_id` в пределах одного секундного decision cycle.
...
```json
{
  ...
  "state_snapshot": {
    ...
    "feature_freshness_sec": 1,
    "feature_window": "rolling_within_last_60s",
    "derived_features": [
      {
...
- публикацию не реже одного раза в секунду на каждый активный инструмент при наличии тикового потока.
...
- создание в том же секундном cycle, где probability evaluation пересекло policy-порог направленного исполнения или зафиксировало безопасный нейтральный/blocked outcome.
...
- Дополнительные окна анализа допускаются только как rolling windows, обновляемые внутри секундного цикла; они не могут быть независимым редким расписанием принятия live-решений.
