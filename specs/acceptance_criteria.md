...
- Для каждого активного инструмента при наличии тикового потока выполняется не реже одного probability refresh в секунду.
...
- Runtime cycle зафиксирован как: tick ingestion по всем USDC futures -> секундный snapshot по каждому активному инструменту -> probability evaluation -> `grid proposal` при пересечении policy-порога.
...
- Любые дополнительные окна анализа допустимы только как внутренние rolling windows внутри секундного цикла, а не как отдельные редкие апдейты decision loop.
...
- На каждый активный инструмент есть проверяемый критерий: не реже одного probability refresh в секунду при наличии тикового потока.
...
- `grid proposal` создаётся только после секундного probability evaluation текущего live cycle.
...
- Старшие таймфреймы выше `1m` не могут быть обязательным входом для live-решения; допустим только контекст, совместимый с ограничением `<=1m`.
