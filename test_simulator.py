...
from runner import BobbyReadiness
...
def test_btc_symbol_override_allows_nonzero_qty():
    cfg = json.loads(Path('config.json').read_text())
    engine = build_risk_engine(cfg)
    plan = engine.build_grid_plan('BTCUSDC', 'LONG_GRID', 84210.9, 50.0, 6)
    assert plan.quantity_per_level > 0
...
def test_foreign_symbol_grid_does_not_replace_inventory_management_context():
    ...
    agent.state = {
        ...
        'open_positions': 1,
        'paper': {
            'inventory': {'side': 'LONG', 'qty': 0.001, 'avg_entry': 84000.0},
            'realized_pnl_usd': 0.0,
            'unrealized_pnl_usd': 0.0,
            'fills_count': 1,
...
if __name__ == '__main__':
    ...
    test_btc_symbol_override_allows_nonzero_qty()
