...
- Переоценка должна происходить в рамках того же секундного decision loop, в котором обновляется probability layer для активного инструмента.
...
- Нельзя опираться на независимые редкие апдейты старших таймфреймов как на обязательный триггер live-переоценки; допустим только контекст, обновляемый внутри секундного цикла и совместимый с `<=1m`.
