...
- Status: active
- Priority: high
- What done: Базовый repeating loop runner собран и локально подтвержден; live polling path через `paper_loop.py --live` реально прогнан на Binance USDS-M; в `risk.py` economics blocker для BTCUSDC усилен явной symbol-aware подсказкой по feasible levels; ложный runtime `DAILY_LOCK` диагностирован и снят через symbol-aware fix в `simulator.py`/`journal.py` (`inventory.symbol`, корректный mark-to-market только по символу позиции), локальный `python3 test_simulator.py` и свежий live probe подтверждают `readiness=PAPER_READY`.
- What left: Оформить runtime automation/hook для регулярного запуска без ручного CLI; отдельно принять symbol-aware policy по BTCUSDC, когда рынок не готов или economics/filter constraints делают сетку нецелесообразной.
- Blockers: Системный cron/runtime hook не подтвержден; BTCUSDC остается условно проблемным символом из-за economics/regime constraints и требует явной runtime policy (skip/reduced levels/allocation) вместо неявных попыток построения сетки.
- Last update: 2026-03-21 11:12 Europe/Moscow
