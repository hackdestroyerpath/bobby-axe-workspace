...
class GridStrategy:
    ...
    def evaluate(self, snapshot: MarketSnapshot, state: dict) -> Decision:
        ...
        levels = self._select_levels(regime)
        spacing = max(
            atr * self.config["grid_spacing_atr"],
            snapshot.last_price * self.config["min_grid_spacing_pct"],
        )
        try:
            plan = self.risk_engine.build_grid_plan(
                snapshot.symbol,
                regime,
                snapshot.mark_price,
                spacing,
                levels,
                directional_tp_spacing_mult=float(self.config["directional_tp_spacing_mult"]),
                directional_sl_spacing_mult=float(self.config["directional_sl_spacing_mult"]),
                neutral_exit_after_bars=int(self.config["neutral_exit_after_bars"]),
            )
        except ValueError as exc:
            return Decision(snapshot.candles[-1].timestamp, snapshot.symbol, "NO_TRADE", regime, "LOW", str(exc), None)

        return Decision(snapshot.candles[-1].timestamp, snapshot.symbol, "GRID_READY", regime, plan.confidence, self._reason(regime, candles, atr_pct, plan), plan)

    def _select_levels(self, regime: str) -> int:
        cap = min(int(self.config.get("grid_levels", 5)), int(self.config.get("grid_levels_max", 5)), int(self.config.get("grid_levels_hard_cap", 7)))
        floor = max(1, int(self.config.get("grid_levels_min", 1)))
        if regime == "NEUTRAL_GRID":
            return max(floor, min(3, cap))
        return max(floor, min(cap, 5))
    ...
    @staticmethod
    def _reason(regime: str, candles: list[Candle], atr_pct: float, plan: GridPlan) -> str:
        closes = [c.close for c in candles[-5:]]
        drift = closes[-1] - closes[0]
        if regime == "LONG_GRID":
            return f"Trend up, TP={plan.take_profit_price}, SL={plan.stop_loss_price}, ATR%={atr_pct:.4f}, drift={drift:.2f}."
        if regime == "SHORT_GRID":
            return f"Trend down, TP={plan.take_profit_price}, SL={plan.stop_loss_price}, ATR%={atr_pct:.4f}, drift={drift:.2f}."
        return f"Range structure detected, exit_after_bars={plan.exit_after_bars}, ATR%={atr_pct:.4f}, drift={drift:.2f}."
