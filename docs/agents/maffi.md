...
- Обязательны usable `analysis_result` для `1m`, `5m`, `60m`.
- Usable `analysis_result` — только объект с `status=ready` и `result_code=ok`.
- `signal=ignore`, а также любые `analysis_result` со `status=partial` или `status=rejected`, можно хранить, но нельзя использовать для построения сетки.
- `signal=neutral` можно учитывать как контекст или подтверждение отсутствия конфликта, но не как самостоятельный directional trigger.
...
```json
{
  ...
  "stop_loss": 63940.0,
  "take_profit": 64780.0,
  "rationale": "5m и 60m подтверждают long-сценарий, 1m используется только для уточнения входа; non-usable анализы исключены из решения.",
  "confidence": 0.72,
  "based_on": {
    "analysis_ids": [
...
- `rationale` должен кратко объяснять, какие usable-сигналы были использованы и почему сетка не нарушает risk framework.
