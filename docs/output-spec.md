...
- `report_job` с метаданными сборки;
- основной PDF от `Jusetta`;
- табличный артефакт (`csv` или `xlsx`) со сводкой по тикерам;
- краткий executive summary от `Bobby Axe` в машиночитаемом и человекочитаемом виде;
- manifest-файл с именами артефактов, `correlation_id`, типом отчёта и временным окном.
...
PDF является обязательным пользовательским артефактом для каждого валидного `report_job`.
...
6. **Artifacts and traceability** — ссылки/пути на связанные артефакты, если они доступны пользователю.
...
`Bobby Axe` обязан выпускать краткий executive summary как самостоятельный блок, который затем вставляется в PDF и сохраняется рядом как отдельный артефакт.
...
- `symbols`;
- список артефактов с типом, путём, checksum и версией;
- статус выпуска;
...
2. `Jusetta` выпускает PDF с обязательными секциями и метаданными;
3. `Bobby Axe` формирует краткий executive summary с decision line;
4. обязательные таблицы и графики включены либо явно заменены exception-логикой, где это разрешено;
5. по каждому тикеру есть каноническая сводка с обязательными полями;
6. все артефакты именованы по детерминированному шаблону и могут быть найдены по `report_type`, периоду и `correlation_id`.
